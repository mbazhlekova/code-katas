...
function tripledouble(num1, num2) {
  ...
  const triple = first.filter(
    (v, k) => v === first[k + 1] && v === first[k + 2]
  );

  const double = second.filter((v, k) => v === second[k + 1]);

  const result = triple.filter(trip => double.find(dub => dub === trip));
